...
public final class GlideExecutor implements ExecutorService {
    ...
    private static final class DefaultThreadFactory implements ThreadFactory {
        ...
        final UncaughtThrowableStrategy f12370a;

        /* renamed from: b, reason: collision with root package name */
        final boolean f12371b;
        private final ThreadFactory delegate;
        private final String name;
        private final AtomicInteger threadNum = new AtomicInteger();

        DefaultThreadFactory(ThreadFactory threadFactory, String str, UncaughtThrowableStrategy uncaughtThrowableStrategy, boolean z2) {
            this.delegate = threadFactory;
            this.name = str;
            this.f12370a = uncaughtThrowableStrategy;
            this.f12371b = z2;
        }
        ...
        public Thread newThread(@NonNull final Runnable runnable) {
            Thread threadNewThread = this.delegate.newThread(new Runnable() { // from class: com.bumptech.glide.load.engine.executor.GlideExecutor.DefaultThreadFactory.1
                ...
                public void run() {
                    if (DefaultThreadFactory.this.f12371b) {
                        StrictMode.setThreadPolicy(new StrictMode.ThreadPolicy.Builder().detectNetwork().penaltyDeath().build());
                    }
                    try {
                        runnable.run();
                    } catch (Throwable th) {
                        DefaultThreadFactory.this.f12370a.handle(th);
                    }
                }
            });
