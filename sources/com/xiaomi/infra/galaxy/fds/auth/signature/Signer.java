...
public class Signer {
    ...
    static String constructStringToSign(HttpMethod httpMethod, URI uri, LinkedListMultimap<String, String> linkedListMultimap) {
        ...
        sb.append("\n");
        sb.append(checkAndGet(linkedListMultimap, "content-md5").get(0));
        sb.append("\n");
        sb.append(checkAndGet(linkedListMultimap, "content-type").get(0));
        sb.append("\n");
        ...
        if (expires > 0) {
            ...
            sb.append(checkAndGet("".equals(checkAndGet(linkedListMultimap, XIAOMI_DATE).get(0)) ? checkAndGet(linkedListMultimap, "date").get(0) : ""));
            sb.append("\n");
        }
    ...
    public static URI generatePresignedUri(String str, String str2, String str3, List<String> list, Date date, HttpMethod httpMethod, String str4, String str5, SignAlgorithm signAlgorithm) throws GalaxyFDSException {
        ...
        try {
            ...
            if (list == null || list.isEmpty()) {
                uri = new URI(uri2.getScheme(), null, uri2.getHost(), uri2.getPort(), "/" + str2 + "/" + str3, "GalaxyAccessKeyId=" + str4 + "&Expires" + ContainerUtils.KEY_VALUE_DELIMITER + date.getTime(), null);
            } else {
                uri = new URI(uri2.getScheme(), null, uri2.getHost(), uri2.getPort(), "/" + str2 + "/" + str3, StringUtils.join(list, "&") + "&GalaxyAccessKeyId" + ContainerUtils.KEY_VALUE_DELIMITER + str4 + "&Expires" + ContainerUtils.KEY_VALUE_DELIMITER + date.getTime(), null);
            }
            return new URI(uri.toString() + "&Signature" + ContainerUtils.KEY_VALUE_DELIMITER + signToBase64(httpMethod, uri, null, str5, signAlgorithm));
        } catch (URISyntaxException e2) {
            LOG.error("Invalid URI syntax", e2);
            throw new GalaxyFDSException("Invalid URI syntax", e2);
        } catch (InvalidKeyException e3) {
            LOG.error("Invalid secret key spec", e3);
            throw new GalaxyFDSException("Invalid secret key spec", e3);
        } catch (NoSuchAlgorithmException e4) {
            LOG.error("Unsupported signature algorithm:" + signAlgorithm, e4);
            throw new GalaxyFDSException("Unsupported signature algorithm:" + signAlgorithm, e4);
        }
    }
    ...
    public static byte[] sign(HttpMethod httpMethod, URI uri, LinkedListMultimap<String, String> linkedListMultimap, String str, SignAlgorithm signAlgorithm) throws NoSuchAlgorithmException, InvalidKeyException {
        ...
        String strConstructStringToSign = constructStringToSign(httpMethod, uri, linkedListMultimap);
        Log log = LOG;
        if (log.isDebugEnabled()) {
            log.debug("Sign for request: " + httpMethod + " " + uri + ", stringToSign=" + strConstructStringToSign);
        }
        Mac mac = Mac.getInstance(signAlgorithm.name());
        mac.init(new SecretKeySpec(str.getBytes(), signAlgorithm.name()));
        return mac.doFinal(strConstructStringToSign.getBytes());
    }
