...
public class MiuiOauth implements XiaomiOAuth {
    ...
    private class MiuiOAuthSession extends FutureTask<Bundle> implements ServiceConnection {
        ...
        boolean bind() throws PackageManager.NameNotFoundException {
            Intent intentBlockGetDefaultIntent = new OAuthServiceManager(MiuiOauth.this.mContext).blockGetDefaultIntent();
            if (intentBlockGetDefaultIntent == null) {
                Log.i(MiuiOauth.TAG, "no xiaomi OAuth service");
                return false;
            }
            boolean zBindService = MiuiOauth.this.mContext.bindService(intentBlockGetDefaultIntent, this, 1);
            if (!zBindService) {
