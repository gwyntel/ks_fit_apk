...
public final class Decoder {
    ...
    public DecoderResult decode(BitMatrix bitMatrix, Map<DecodeHintType, ?> map) throws ChecksumException, FormatException {
        byte[] bArr;
        byte[] bArrA = new BitMatrixParser(bitMatrix).a();
        correctErrors(bArrA, 0, 10, 10, 0);
        int i2 = bArrA[0] & 15;
        if (i2 == 2 || i2 == 3 || i2 == 4) {
            correctErrors(bArrA, 20, 84, 40, 1);
            correctErrors(bArrA, 20, 84, 40, 2);
            bArr = new byte[94];
        } else {
            if (i2 != 5) {
                throw FormatException.getFormatInstance();
            }
            correctErrors(bArrA, 20, 68, 56, 1);
            correctErrors(bArrA, 20, 68, 56, 2);
            bArr = new byte[78];
        }
        System.arraycopy(bArrA, 0, bArr, 0, 10);
        System.arraycopy(bArrA, 20, bArr, 10, bArr.length - 10);
        return DecodedBitStreamParser.a(bArr, i2);
    }
}
