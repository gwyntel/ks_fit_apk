...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(@NotNull DispatchedTask<? super T> dispatchedTask, int i2) {
        ...
        CoroutineContext f25479a = delegate$kotlinx_coroutines_core.getF25396a();
        if (coroutineDispatcher.isDispatchNeeded(f25479a)) {
            coroutineDispatcher.mo2380dispatch(f25479a, dispatchedTask);
        } else {
    ...
    public static final <T> void resume(@NotNull DispatchedTask<? super T> dispatchedTask, @NotNull Continuation<? super T> continuation, boolean z2) {
        ...
        Object objM837constructorimpl = Result.m837constructorimpl(successfulResult$kotlinx_coroutines_core);
        if (!z2) {
            continuation.resumeWith(objM837constructorimpl);
            return;
        }
        ...
        CoroutineContext f25479a = continuation2.getF25396a();
        Object objUpdateThreadContext = ThreadContextKt.updateThreadContext(f25479a, obj);
        UndispatchedCoroutine<?> undispatchedCoroutineUpdateUndispatchedCompletion = objUpdateThreadContext != ThreadContextKt.NO_THREAD_ELEMENTS ? CoroutineContextKt.updateUndispatchedCompletion(continuation2, f25479a, objUpdateThreadContext) : null;
        try {
            dispatchedContinuation.continuation.resumeWith(objM837constructorimpl);
            Unit unit = Unit.INSTANCE;
        } finally {
            if (undispatchedCoroutineUpdateUndispatchedCompletion == null || undispatchedCoroutineUpdateUndispatchedCompletion.clearThreadContext()) {
                ThreadContextKt.restoreThreadContext(f25479a, objUpdateThreadContext);
            }
        }
    }
    ...
    public static final void resumeWithStackTrace(@NotNull Continuation<?> continuation, @NotNull Throwable th) {
        Result.Companion companion = Result.INSTANCE;
        continuation.resumeWith(Result.m837constructorimpl(ResultKt.createFailure(th)));
    }
