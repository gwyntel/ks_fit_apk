...
public class KeyFactorySpi extends BaseKeyFactorySpi implements c {
    ...
    public PrivateKey generatePrivate(a aVar) throws IOException {
        C0364n algorithm = aVar.c().getAlgorithm();
        if (algorithm.equals(X9ObjectIdentifiers.id_ecPublicKey)) {
            return new BCECPrivateKey(this.algorithm, aVar, this.configuration);
        }
        throw new IOException("algorithm identifier " + algorithm + " in key not recognised");
    }

    @Override // c.a.c.a.c.c
    public PublicKey generatePublic(c.a.a.j.b bVar) throws IOException {
        C0364n algorithm = bVar.getAlgorithm().getAlgorithm();
        if (algorithm.equals(X9ObjectIdentifiers.id_ecPublicKey)) {
            return new BCECPublicKey(this.algorithm, bVar, this.configuration);
        }
        throw new IOException("algorithm identifier " + algorithm + " in key not recognised");
    }
}
