...
public class ConstructorUtils {
    ...
    public static Constructor getMatchingAccessibleConstructor(Class cls, Class[] clsArr) throws NoSuchMethodException, SecurityException {
        ...
        try {
            ...
            MemberUtils.f(constructor);
            return constructor;
        } catch (NoSuchMethodException unused) {
            Constructor<?>[] constructors = cls.getConstructors();
            Constructor constructor2 = null;
            for (int i2 = 0; i2 < constructors.length; i2++) {
                if (ClassUtils.isAssignable(clsArr, (Class[]) constructors[i2].getParameterTypes(), true) && (accessibleConstructor = getAccessibleConstructor(constructors[i2])) != null) {
                    MemberUtils.f(accessibleConstructor);
                    if (constructor2 == null || MemberUtils.b(accessibleConstructor.getParameterTypes(), constructor2.getParameterTypes(), clsArr) < 0) {
    ...
    public static Constructor getAccessibleConstructor(Constructor constructor) {
        if (MemberUtils.c(constructor) && Modifier.isPublic(constructor.getDeclaringClass().getModifiers())) {
            return constructor;
        }
        return null;
    }
