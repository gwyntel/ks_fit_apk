...
public class FileAlterationObserver implements Serializable {
    ...
    private FileEntry[] doListFiles(File file, FileEntry fileEntry) {
        ...
        for (int i2 = 0; i2 < fileArrListFiles.length; i2++) {
            fileEntryArr[i2] = createFileEntry(fileEntry, fileArrListFiles[i2]);
        }
        return fileEntryArr;
    }
    ...
    private void checkAndNotify(FileEntry fileEntry, FileEntry[] fileEntryArr, File[] fileArr) {
        ...
        for (FileEntry fileEntry2 : fileEntryArr) {
            while (i2 < fileArr.length && this.comparator.compare(fileEntry2.getFile(), fileArr[i2]) > 0) {
                FileEntry fileEntryCreateFileEntry = createFileEntry(fileEntry, fileArr[i2]);
                fileEntryArr2[i2] = fileEntryCreateFileEntry;
        ...
        while (i2 < fileArr.length) {
            FileEntry fileEntryCreateFileEntry2 = createFileEntry(fileEntry, fileArr[i2]);
            fileEntryArr2[i2] = fileEntryCreateFileEntry2;
