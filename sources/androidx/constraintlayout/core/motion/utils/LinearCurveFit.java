...
public class LinearCurveFit extends CurveFit {
    ...
    public LinearCurveFit(double[] dArr, double[][] dArr2) {
        ...
        this.f2716a = new double[length2];
        this.mT = dArr;
        this.mY = dArr2;
        if (length2 <= 2) {
            return;
    ...
    public void getPos(double d2, double[] dArr) {
        ...
        if (this.mExtrapolate) {
            double d3 = dArr2[0];
            if (d2 <= d3) {
                getSlope(d3, this.f2716a);
                for (int i3 = 0; i3 < length2; i3++) {
                    dArr[i3] = this.mY[0][i3] + ((d2 - this.mT[0]) * this.f2716a[i3]);
                }
                return;
            }
            int i4 = length - 1;
            double d4 = dArr2[i4];
            if (d2 >= d4) {
                getSlope(d4, this.f2716a);
                while (i2 < length2) {
                    dArr[i2] = this.mY[i4][i2] + ((d2 - this.mT[i4]) * this.f2716a[i2]);
                    i2++;
                }
                return;
            }
    ...
    public void getPos(double d2, float[] fArr) {
        ...
        if (this.mExtrapolate) {
            ...
            if (d2 <= d3) {
                getSlope(d3, this.f2716a);
                for (int i3 = 0; i3 < length2; i3++) {
                    fArr[i3] = (float) (this.mY[0][i3] + ((d2 - this.mT[0]) * this.f2716a[i3]));
                }
                return;
            }
            int i4 = length - 1;
            ...
            if (d2 >= d4) {
                getSlope(d4, this.f2716a);
                while (i2 < length2) {
                    fArr[i2] = (float) (this.mY[i4][i2] + ((d2 - this.mT[i4]) * this.f2716a[i2]));
                    i2++;
                }
                return;
            }
